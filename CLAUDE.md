...
- `src/modules/<name>/` — feature slice: `*.routes.ts` + `*.controllers.ts`. System routes are unauthenticated; others sit behind auth. `report` (`POST /v1/report`, renamed from `/events`) serves the SAR-detection query the Report tab/map use; `regions` serves EEZ/MPA/coastline boundary geometry (context-layer map toggles); `vessels` wraps the GFW Vessels API for the Vessel tab's search and on-demand vessel-identity enrichment; `events` (`POST /v1/events/search`) wraps the GFW Events API (encounters/loitering/port-visits/gaps/fishing) for the Event tab's search, independent of the Report tab/map — same thin pass-through shape as `vessels`, see `docs/tech/event-search.md`; `mcp` exposes a Streamable HTTP MCP server (`POST /v1/mcp`) so an AI agent can call Pharos as tools — gated by per-agent API keys (`MCP_API_KEYS`, a `label -> token` JSON map; one entry per person/agent, revocable independently), not the user JWT flow; see `docs/knowledge/mcp-notes.md`.
...
### Auth
Backend issues a short-lived **access** JWT and a long-lived **refresh** JWT, both signed with `JWT_SECRET`. Frontend attaches the access token and silently refreshes on expiry. Protected endpoints (e.g. `/v1/report`) reject requests without a valid access token. See `docs/api/authentication.md`.
...
- Domain model: an **"event"** is a SAR detection carrying scoring, hotspot context, and context layers (EEZ / MPA / Bathymetry). Core types live in `packages/types/src/eventTypes.ts`; enums (`EConfidenceTiers`, `EHotspotStrength`, `EReasonCodes`, `EContextLayers`) in `packages/enum/src/`. Sample/dev data: `src/helpers/fixtures/`. This is a **different "event" than the Event tab's**: the Event tab searches GFW's own Events API (encounters/loitering/port-visits/gaps/fishing — `TGlobalEvent`, `EEventType`, `EEventDatasets` in `packages/types`/`packages/enum`'s `gfwTypes.ts`/`gfwEnum.ts`), a vessel-behavior record, not a SAR detection. To avoid the name collision, its frontend stores are prefixed `gfwEvent*` (`gfwEventSearchStore.ts`, `gfwEventStore.ts`) — the SAR-detection store (`eventStore.ts`/`useEventStore`) keeps the unprefixed name since it's the older, far-more-referenced one. Backend has no such collision (SAR detections are named `report`/`Detection*` throughout, e.g. `DetectionService`, `repositories/detection`), so the Events API module/service/repository there use the plain `event` name — see `docs/tech/event-search.md`.
...
## Project notes
- Personal planning/AI-leverage notes live under `docs/planning/` and `docs/knowledge/`, which are **git-ignored** — they won't appear in `git status` or a fresh clone, so check the working tree before assuming they're absent. The product proposal is a single file, `docs/product/proposal.md` — no version suffix; edit it in place and rely on git history as the version trail, rather than creating `Proposal_V*.md` copies.
