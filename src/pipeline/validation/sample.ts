...
import { FeatureCollection, IMultiPolygonGeometry } from "../../types/geoJSONTypes";
import { EValidationFailureMode, EValidationLabel, ILandPolygonProperties, IValidationResp, IValidationSample, TValidationGeoJSON } from "../../types/validationTypes";
...
import { I4wingsAPIResponse, I4wingsReportGetURLParams, I4wingsReportPostBodyParams, I4wingsReportPostURLParams, IEventGetURLParams, T4wingsSource, TEventSource } from "../../types/gfwTypes";
...
export const isOnLand = (a_LandPolygons: FeatureCollection<IMultiPolygonGeometry, ILandPolygonProperties>, a_Lon: number, a_Lat: number): boolean => {
...
export const createValidationSample = (a_EventSchema: IEventSchema): IValidationSample => {

  const landPolygons = readLandPolygons()
  const isEventOnLand = isOnLand(landPolygons, a_EventSchema.lon, a_EventSchema.lat)
  return {
...
export const generateValidationGeoJSON = (a_ValidationSample: IValidationSample): TValidationGeoJSON => {
  ...
  return {
    ...
    geometry: {
      type: "Point",
      coordinates: [
        a_ValidationSample.lon,
        a_ValidationSample.lat
      ]
    },
...
export const getValidationSamples = async (
  ...
  const entries4wings = getEntriesFrom4wingsResponse(resp4wings.results, a_Source as T4wingsSource)
  ...
  log(`Creating event schemas, no. entry: ${reducedEntriesNr.length}...`, '', ELogLevel.message, '3');
