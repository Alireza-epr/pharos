...
export const generateHotspots = (a_Events: IEventSchema[], a_Resolution: number) => {
    ...
    let hotspots: IHotspot[] = Array.from(h3Indexes).map(([key, events]) => {
        ...
        for(const event of events){
            ...
            if(!event.matched_flag && event.scoring.triage_score !== null && event.scoring.triage_score > config.threshold.low_detection_confidence_threshold){
            ...
            if(event.distance_to_coast_km !== null && event.distance_to_coast_km <= config.threshold.near_coast_threshold){
                nearCoastCount++
            }
        ...
        return {
            ...
            count_unmatched,
            count_high_score_unmatched,
            mean_score: scoreCount > 0 ? parseFloat((sumScore / scoreCount).toFixed(2)) : null,
            mean_uncertainty: uncertaintyCount > 0 ? parseFloat((sumUncertainty / uncertaintyCount).toFixed(2)) : null,
            pct_near_coast: parseFloat(((nearCoastCount / events.length) * 100).toFixed(2)),
            recurrence_count,
            days_with_unmatched
        }
    })
    ...
    const recurrenceMap = new Map<string, { recurrence_count: number; days_with_unmatched: number }>()
    for (const [cell_id, hs] of groupedHotspots) {
        recurrenceMap.set(cell_id, {
            recurrence_count: hs.map(h => h.count_unmatched).reduce((a, b) => a + b, 0),
...
export const getMeanScore = (a_Events: IEventSchema[]) => {
    const validTriage = a_Events.map(e => e.scoring.triage_score).filter(s => s !== null)
    const sumTriage = validTriage.reduce((a, b) => a + b)
    const mean_score = parseFloat((sumTriage / validTriage.length).toFixed(2))

    const validUncertainty = a_Events.map(e => e.scoring.uncertainty_score).filter(s => s !== null)
    const sumUncertainty = validUncertainty.reduce((a, b) => a + b)
    const mean_uncertainty = parseFloat((sumUncertainty / validUncertainty.length).toFixed(2))
...
export const featureFromHotspot = (a_Hotspots: IHotspot[]): IFeature<IPolygonGeometry, IHotspot>[] => {
