...
| detections            | integer          | number of SAR detection events in that grid cell and time bucket that were matched to that vessel (if matched), or to no vessel (if unmatched). | If ‘Matched’, count detections of the same vessel multiple times. If ‘Unmatched’, identity is unknown (same or different object). |
