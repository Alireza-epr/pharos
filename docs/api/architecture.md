...
- **Repository**: Handles communication with resources such as APIs, data stores, or notification services. Each repository hides one endpoint's syntax behind generic verbs (read / write / fetch / …), so swapping the backend is a new implementation, not a service change. Implemented for the serving slice as `repositories/serving` (`ParquetServingRepository` behind `IServingRepository` — the contract lives in `helpers/types/serviceTypes.ts` so service and repository share the abstraction without a cycle — plus a config-selectable factory; Parquet today, DB/object-store tomorrow), for the provider as `repositories/detection` (`GfwDetectionRepository` behind `IDetectionRepository`, consumed by `DetectionService`; swap the SAR/AIS source without touching the service), and for vessel identity as `repositories/vessel` (`GfwVesselRepository` behind `IVesselRepository`'s `search`/`list` verbs, consumed by `VesselService` — a thin pass-through with no caching/scoring layer, unlike `DetectionService`), and for behavioral events (encounters/loitering/port-visits/gaps/fishing) as `repositories/event` (`GfwEventRepository` behind `IEventRepository`'s single `search` verb, consumed by `EventService` — same thin-pass-through shape as vessel identity, backing the Event tab).
- **External Systems**: Integrations such as APIs (GFW), data stores (Parquet/DB), email, or notification services.
