import { EEventType } from '@packages/enum';
import {
  I4wingsAPIResponse,
  I4wingsEntry,
  IEventAPIResponse,
  IPortVisitEvent,
} from '@packages/types';

export const api4wingsResponse: I4wingsAPIResponse = {
  total: 1,
  limit: null,
  offset: null,
  nextOffset: null,
  metadata: {},
  entries: [
    {
      'public-global-sar-presence:v3.0': [
        {
          callsign: 'V2OW7',
          dataset: 'public-global-vessel-identity:v3.0',
          date: '2025-12-04 16:00',
          detections: 1,
          entryTimestamp: '2025-12-04T16:53:26Z',
          exitTimestamp: '2025-12-04T16:53:26Z',
          firstTransmissionDate: '2012-01-02T16:28:28Z',
          flag: 'ATG',
          geartype: 'CARGO',
          imo: '9287807',
          lastTransmissionDate: '2026-02-25T23:56:08Z',
          lat: 55.22,
          lon: 14.40999984741211,
          mmsi: '305773000',
          shipName: 'VOHBURG',
          vesselId: '369fc1e02-2678-b669-af58-b2f3ae66a515',
          vesselType: 'CARGO',
        },
        {
          callsign: '5BNV4',
          dataset: 'public-global-vessel-identity:v3.0',
          date: '2025-12-06 05:00',
          detections: 1,
          entryTimestamp: '2025-12-06T05:25:03Z',
          exitTimestamp: '2025-12-06T05:25:03Z',
          firstTransmissionDate: '2016-08-30T13:30:09Z',
          flag: 'CYP',
          geartype: 'CARGO',
          imo: '9552044',
          lastTransmissionDate: '2026-02-25T23:58:10Z',
          lat: 55.09,
          lon: 14.09000015258789,
          mmsi: '212584000',
          shipName: 'MIRAMAR',
          vesselId: 'afdb60bd6-6d97-eb03-ef84-c1e3b26c10cc',
          vesselType: 'CARGO',
        },
        {
          callsign: 'PBDI',
          dataset: 'public-global-vessel-identity:v3.0',
          date: '2025-12-06 05:00',
          detections: 1,
          entryTimestamp: '2025-12-06T05:25:03Z',
          exitTimestamp: '2025-12-06T05:25:03Z',
          firstTransmissionDate: '2025-10-14T12:26:31Z',
          flag: 'NLD',
          geartype: 'CARGO',
          imo: '1046104',
          lastTransmissionDate: '2026-02-25T23:59:59Z',
          lat: 55.24,
          lon: 14.329999923706055,
          mmsi: '244067000',
          shipName: 'BALTIC SAIL',
          vesselId: '43c5ec9e1-1026-c273-d280-f0c34b520f53',
          vesselType: 'CARGO',
        },
        {
          callsign: 'DFXF',
          dataset: 'public-global-vessel-identity:v3.0',
          date: '2025-12-04 16:00',
          detections: 1,
          entryTimestamp: '2025-12-04T16:53:26Z',
          exitTimestamp: '2025-12-04T16:53:26Z',
          firstTransmissionDate: '2014-07-30T09:19:45Z',
          flag: 'DEU',
          geartype: 'CARGO',
          imo: '9196254',
          lastTransmissionDate: '2026-02-25T23:59:39Z',
          lat: 55.16,
          lon: 14.170000076293945,
          mmsi: '218797000',
          shipName: 'VAERMLAND',
          vesselId: 'b867aac71-181e-e3fb-ef3b-1267b81caedc',
          vesselType: 'CARGO',
        },
        {
          callsign: 'OURV2',
          dataset: 'public-global-vessel-identity:v3.0',
          date: '2025-12-06 05:00',
          detections: 1,
          entryTimestamp: '2025-12-06T05:25:03Z',
          exitTimestamp: '2025-12-06T05:25:03Z',
          firstTransmissionDate: '2012-01-01T00:03:55Z',
          flag: 'DNK',
          geartype: 'CARGO',
          imo: '8104565',
          lastTransmissionDate: '2026-02-25T23:59:41Z',
          lat: 55.23,
          lon: 14.1899995803833,
          mmsi: '219002392',
          shipName: 'AMANDA',
          vesselId: 'df3528985-5287-a35b-89a0-b2e9c1e6db13',
          vesselType: 'CARGO',
        },
        {
          callsign: '5LYK8',
          dataset: 'public-global-vessel-identity:v3.0',
          date: '2025-12-04 16:00',
          detections: 1,
          entryTimestamp: '2025-12-04T16:53:26Z',
          exitTimestamp: '2025-12-04T16:53:26Z',
          firstTransmissionDate: '2025-10-31T03:06:26Z',
          flag: 'LBR',
          geartype: 'OTHER',
          imo: '9312860',
          lastTransmissionDate: '2026-02-25T23:59:56Z',
          lat: 55.15,
          lon: 14.220000267028809,
          mmsi: '636025638',
          shipName: 'IONIAN SUN',
          vesselId: 'ee20f2f54-4c41-ce6c-ce45-fa136fb6e2cc',
          vesselType: 'OTHER',
        },
        {
          callsign: '8PXO8',
          dataset: 'public-global-vessel-identity:v3.0',
          date: '2025-12-04 16:00',
          detections: 1,
          entryTimestamp: '2025-12-04T16:53:26Z',
          exitTimestamp: '2025-12-04T16:53:26Z',
          firstTransmissionDate: '2024-03-20T02:01:54Z',
          flag: 'BRB',
          geartype: 'OTHER',
          imo: '9298662',
          lastTransmissionDate: '2026-02-25T23:59:42Z',
          lat: 55.16,
          lon: 14.109999656677246,
          mmsi: '314842000',
          shipName: 'MULE',
          vesselId: 'de435d0f3-306c-2869-d119-fff03a975f8f',
          vesselType: 'OTHER',
        },
        {
          callsign: 'OXPQ2',
          dataset: 'public-global-vessel-identity:v3.0',
          date: '2025-12-04 16:00',
          detections: 1,
          entryTimestamp: '2025-12-04T16:53:26Z',
          exitTimestamp: '2025-12-04T16:53:26Z',
          firstTransmissionDate: '2018-05-17T13:01:39Z',
          flag: 'DNK',
          geartype: 'PASSENGER',
          imo: '9812107',
          lastTransmissionDate: '2026-02-25T23:58:01Z',
          lat: 55.19,
          lon: 14.239999771118164,
          mmsi: '219026000',
          shipName: 'HAMMERSHUS',
          vesselId: '2cb75b670-08a6-fc17-8fd9-5d9d10a0fbdf',
          vesselType: 'PASSENGER',
        },
        {
          callsign: '',
          dataset: '',
          date: '2025-12-04 16:00',
          detections: 1,
          entryTimestamp: '2025-12-04T16:53:26Z',
          exitTimestamp: '2025-12-04T16:53:26Z',
          firstTransmissionDate: '',
          flag: '',
          geartype: '',
          imo: '',
          lastTransmissionDate: '',
          lat: 55.2,
          lon: 14.210000038146973,
          mmsi: '',
          shipName: '',
          vesselId: '',
          vesselType: '',
        },
        {
          callsign: '5BRS5',
          dataset: 'public-global-vessel-identity:v3.0',
          date: '2025-12-06 05:00',
          detections: 1,
          entryTimestamp: '2025-12-06T05:25:03Z',
          exitTimestamp: '2025-12-06T05:25:03Z',
          firstTransmissionDate: '2021-02-02T09:18:42Z',
          flag: 'CYP',
          geartype: 'CARGO',
          imo: '9213703',
          lastTransmissionDate: '2026-02-25T23:31:26Z',
          lat: 55.21,
          lon: 14.40999984741211,
          mmsi: '209982000',
          shipName: 'NINA B',
          vesselId: 'e4a2ba1ec-c786-5395-6e67-1fb8e1abe320',
          vesselType: 'CARGO',
        },
        {
          callsign: '8POQ',
          dataset: 'public-global-vessel-identity:v3.0',
          date: '2025-12-06 05:00',
          detections: 1,
          entryTimestamp: '2025-12-06T05:25:03Z',
          exitTimestamp: '2025-12-06T05:25:03Z',
          firstTransmissionDate: '2025-10-25T08:35:19Z',
          flag: 'BRB',
          geartype: 'OTHER',
          imo: '9318096',
          lastTransmissionDate: '2026-02-25T23:59:57Z',
          lat: 55.15,
          lon: 14.300000190734863,
          mmsi: '314001147',
          shipName: 'COSMO SAIL',
          vesselId: 'ddd3f8ebb-b915-bcf6-9d8f-5f463eb99b7f',
          vesselType: 'OTHER',
        },
        {
          callsign: 'OVDN',
          dataset: 'public-global-vessel-identity:v3.0',
          date: '2025-12-04 16:00',
          detections: 1,
          entryTimestamp: '2025-12-04T16:53:26Z',
          exitTimestamp: '2025-12-04T16:53:26Z',
          firstTransmissionDate: '2018-09-13T16:47:49Z',
          flag: 'DNK',
          geartype: 'OTHER',
          imo: '',
          lastTransmissionDate: '2026-02-11T17:57:20Z',
          lat: 55.16,
          lon: 14.680000305175781,
          mmsi: '219000217',
          shipName: 'HDMS SOELOEVEN',
          vesselId: '22c7ca42d-df50-18b5-713a-a81e9115052c',
          vesselType: 'OTHER',
        },
        {
          callsign: 'YDJY3',
          dataset: 'public-global-vessel-identity:v3.0',
          date: '2025-12-06 05:00',
          detections: 1,
          entryTimestamp: '2025-12-06T05:25:03Z',
          exitTimestamp: '2025-12-06T05:25:03Z',
          firstTransmissionDate: '2024-05-08T11:07:06Z',
          flag: 'IDN',
          geartype: 'OTHER',
          imo: '9388730',
          lastTransmissionDate: '2026-02-25T23:59:14Z',
          lat: 55.23,
          lon: 14.289999961853027,
          mmsi: '525121076',
          shipName: 'SAVIR TIGER',
          vesselId: '403dc1002-210e-de4b-331d-b4012153dfa4',
          vesselType: 'OTHER',
        },
      ],
    },
  ],
};

export const api4wingsResponse_bad_coordinates: any = {
  total: 1,
  limit: null,
  offset: null,
  nextOffset: null,
  metadata: {},
  entries: [
    {
      'public-global-sar-presence:v3.0': [
        {
          callsign: 'V2OW7',
          dataset: 'public-global-vessel-identity:v3.0',
          date: '2025-12-04 16:00',
          detections: 1,
          entryTimestamp: '2025-12-04T16:53:26Z',
          exitTimestamp: '2025-12-04T16:53:26Z',
          firstTransmissionDate: '2012-01-02T16:28:28Z',
          flag: 'ATG',
          geartype: 'CARGO',
          imo: '9287807',
          lastTransmissionDate: '2026-02-25T23:56:08Z',
          lat: '55.22',
          lon: 14.40999984741211,
          mmsi: '305773000',
          shipName: 'VOHBURG',
          vesselId: '369fc1e02-2678-b669-af58-b2f3ae66a515',
          vesselType: 'CARGO',
        },
        {
          callsign: 'V2OW7',
          dataset: 'public-global-vessel-identity:v3.0',
          date: '2025-12-04 16:00',
          detections: 1,
          entryTimestamp: '2025-12-04T16:53:26Z',
          exitTimestamp: '2025-12-04T16:53:26Z',
          firstTransmissionDate: '2012-01-02T16:28:28Z',
          flag: 'ATG',
          geartype: 'CARGO',
          imo: '9287807',
          lastTransmissionDate: '2026-02-25T23:56:08Z',
          lat: 55.22,
          lon: NaN,
          mmsi: '305773000',
          shipName: 'VOHBURG',
          vesselId: '369fc1e02-2678-b669-af58-b2f3ae66a515',
          vesselType: 'CARGO',
        },
        {
          callsign: 'V2OW7',
          dataset: 'public-global-vessel-identity:v3.0',
          date: '2025-12-04 16:00',
          detections: 1,
          entryTimestamp: '2025-12-04T16:53:26Z',
          exitTimestamp: '2025-12-04T16:53:26Z',
          firstTransmissionDate: '2012-01-02T16:28:28Z',
          flag: 'ATG',
          geartype: 'CARGO',
          imo: '9287807',
          lastTransmissionDate: '2026-02-25T23:56:08Z',
          lat: 91,
          lon: 20,
          mmsi: '305773000',
          shipName: 'VOHBURG',
          vesselId: '369fc1e02-2678-b669-af58-b2f3ae66a515',
          vesselType: 'CARGO',
        },
      ],
    },
  ],
};

export const api4wingsResponse_bad_vessel_type: any = {
  total: 1,
  limit: null,
  offset: null,
  nextOffset: null,
  metadata: {},
  entries: [
    {
      'public-global-sar-presence:v3.0': [
        {
          callsign: 'V2OW7',
          dataset: 'public-global-vessel-identity:v3.0',
          date: '2025-12-04 16:00',
          detections: 1,
          entryTimestamp: '2025-12-04T16:53:26Z',
          exitTimestamp: '2025-12-04T16:53:26Z',
          firstTransmissionDate: '2012-01-02T16:28:28Z',
          flag: 'ATG',
          geartype: 'CARGO',
          imo: '9287807',
          lastTransmissionDate: '2026-02-25T23:56:08Z',
          lat: '55.22',
          lon: 14.40999984741211,
          mmsi: '305773000',
          shipName: 'VOHBURG',
          vesselId: '369fc1e02-2678-b669-af58-b2f3ae66a515',
          vesselType: 'UNKNOWN',
        },
        {
          callsign: 'V2OW7',
          dataset: 'public-global-vessel-identity:v3.0',
          date: '2025-12-04 16:00',
          detections: 1,
          entryTimestamp: '2025-12-04T16:53:26Z',
          exitTimestamp: '2025-12-04T16:53:26Z',
          firstTransmissionDate: '2012-01-02T16:28:28Z',
          flag: 'ATG',
          geartype: 'CARGO',
          imo: '9287807',
          lastTransmissionDate: '2026-02-25T23:56:08Z',
          lat: 55.22,
          lon: NaN,
          mmsi: '305773000',
          shipName: 'VOHBURG',
          vesselId: '369fc1e02-2678-b669-af58-b2f3ae66a515',
          vesselType: 2,
        },
        {
          callsign: 'V2OW7',
          dataset: 'public-global-vessel-identity:v3.0',
          date: '2025-12-04 16:00',
          detections: 1,
          entryTimestamp: '2025-12-04T16:53:26Z',
          exitTimestamp: '2025-12-04T16:53:26Z',
          firstTransmissionDate: '2012-01-02T16:28:28Z',
          flag: 'ATG',
          geartype: 'CARGO',
          imo: '9287807',
          lastTransmissionDate: '2026-02-25T23:56:08Z',
          lat: 91,
          lon: 20,
          mmsi: '305773000',
          shipName: 'VOHBURG',
          vesselId: '369fc1e02-2678-b669-af58-b2f3ae66a515',
          vesselType: 'CARGOS',
        },
      ],
    },
  ],
};

export const api4wingsResponse_bad_date: any = {
  total: 1,
  limit: null,
  offset: null,
  nextOffset: null,
  metadata: {},
  entries: [
    {
      'public-global-sar-presence:v3.0': [
        {
          callsign: 'V2OW7',
          dataset: 'public-global-vessel-identity:v3.0',
          date: '2025-12-04', //DAILY
          detections: 1,
          entryTimestamp: '2025-12-04T16:53:26Z',
          exitTimestamp: '2025-12-04T16:53:26Z',
          firstTransmissionDate: '2012-01-02T16:28:28Z',
          flag: 'ATG',
          geartype: 'CARGO',
          imo: '9287807',
          lastTransmissionDate: '2026-02-25T23:56:08Z',
          lat: 55,
          lon: 14.40999984741211,
          mmsi: '305773000',
          shipName: 'VOHBURG',
          vesselId: '369fc1e02-2678-b669-af58-b2f3ae66a515',
          vesselType: 'CARGO',
        },
        {
          callsign: 'V2OW7',
          dataset: 'public-global-vessel-identity:v3.0',
          date: '2025-12-01T00:00:00Z,2025-12-07T23:59:59Z', //ENTIRE
          detections: 1,
          entryTimestamp: '2025-12-04T16:53:26Z',
          exitTimestamp: '2025-12-04T16:53:26Z',
          firstTransmissionDate: '2012-01-02T16:28:28Z',
          flag: 'ATG',
          geartype: 'CARGO',
          imo: '9287807',
          lastTransmissionDate: '2026-02-25T23:56:08Z',
          lat: 55.22,
          lon: 12,
          mmsi: '305773000',
          shipName: 'VOHBURG',
          vesselId: '369fc1e02-2678-b669-af58-b2f3ae66a515',
          vesselType: 'CARGO',
        },
        {
          callsign: 'V2OW7',
          dataset: 'public-global-vessel-identity:v3.0',
          date: '2025-12', //MONTHLY
          detections: 1,
          entryTimestamp: '2025-12-04T16:53:26Z',
          exitTimestamp: '2025-12-04T16:53:26Z',
          firstTransmissionDate: '2012-01-02T16:28:28Z',
          flag: 'ATG',
          geartype: 'CARGOS',
          imo: '9287807',
          lastTransmissionDate: '2026-02-25T23:56:08Z',
          lat: 91,
          lon: 20,
          mmsi: '305773000',
          shipName: 'VOHBURG',
          vesselId: '369fc1e02-2678-b669-af58-b2f3ae66a515',
          vesselType: 'CARGOS',
        },
        {
          callsign: 'V2OW7',
          dataset: 'public-global-vessel-identity:v3.0',
          date: '2025', //YEARLY
          detections: 1,
          entryTimestamp: '2025-12-04T16:53:26Z',
          exitTimestamp: '2025-12-04T16:53:26Z',
          firstTransmissionDate: '2012-01-02T16:28:28Z',
          flag: 'ATG',
          geartype: 'CARGO',
          imo: '9287807',
          lastTransmissionDate: '2026-02-25T23:56:08Z',
          lat: 20,
          lon: 20,
          mmsi: '305773000',
          shipName: 'VOHBURG',
          vesselId: '369fc1e02-2678-b669-af58-b2f3ae66a515',
          vesselType: 'CARGO',
        },
      ],
    },
  ],
};

export const api4wingsResponse_bad_multi: any = {
  total: 1,
  limit: null,
  offset: null,
  nextOffset: null,
  metadata: {},
  entries: [
    {
      'public-global-sar-presence:v3.0': [
        {
          callsign: 'V2OW7',
          dataset: 'public-global-vessel-identity:v3.0',
          date: '2025-12-04', //DAILY
          detections: 1,
          entryTimestamp: '2025-12-04T16:53:26Z',
          exitTimestamp: '2025-12-04T16:53:26Z',
          firstTransmissionDate: '2012-01-02T16:28:28Z',
          flag: 'ATG',
          geartype: 'CARGO',
          imo: '9287807',
          lastTransmissionDate: '2026-02-25T23:56:08Z',
          lat: '55',
          lon: 14.40999984741211,
          mmsi: '305773000',
          shipName: 'VOHBURG',
          vesselId: '369fc1e02-2678-b669-af58-b2f3ae66a515',
          vesselType: 'CARG',
        },
        {
          callsign: 'V2OW7',
          dataset: 'public-global-vessel-identity:v3.0',
          date: '2025-12-01T00:00:00Z,2025-12-07T23:59:59Z', //ENTIRE
          detections: 1,
          entryTimestamp: '2025-12-04T16:53:26Z',
          exitTimestamp: '2025-12-04T16:53:26Z',
          firstTransmissionDate: '2012-01-02T16:28:28Z',
          flag: 'ATG',
          geartype: 'CARGO',
          imo: '9287807',
          lastTransmissionDate: '2026-02-25T23:56:08Z',
          lat: 55.22,
          lon: NaN,
          mmsi: '305773000',
          shipName: 'VOHBURG',
          vesselId: '369fc1e02-2678-b669-af58-b2f3ae66a515',
          vesselType: 'ARGO',
        },
        {
          callsign: 'V2OW7',
          dataset: 'public-global-vessel-identity:v3.0',
          date: '2025-12', //MONTHLY
          detections: 1,
          entryTimestamp: '2025-12-04T16:53:26Z',
          exitTimestamp: '2025-12-04T16:53:26Z',
          firstTransmissionDate: '2012-01-02T16:28:28Z',
          flag: 'ATG',
          geartype: 'CARGOS',
          imo: '9287807',
          lastTransmissionDate: '2026-02-25T23:56:08Z',
          lat: undefined,
          lon: null,
          mmsi: '305773000',
          shipName: 'VOHBURG',
          vesselId: '369fc1e02-2678-b669-af58-b2f3ae66a515',
          vesselType: 55,
        },
        {
          callsign: 'V2OW7',
          dataset: 'public-global-vessel-identity:v3.0',
          date: '2025', //YEARLY
          detections: 1,
          entryTimestamp: '2025-12-04T16:53:26Z',
          exitTimestamp: '2025-12-04T16:53:26Z',
          firstTransmissionDate: '2012-01-02T16:28:28Z',
          flag: 'ATG',
          geartype: 'CARGO',
          imo: '9287807',
          lastTransmissionDate: '2026-02-25T23:56:08Z',
          lat: 1000,
          lon: 210,
          mmsi: '305773000',
          shipName: 'VOHBURG',
          vesselId: '369fc1e02-2678-b669-af58-b2f3ae66a515',
          vesselType: 'N/A',
        },
      ],
    },
  ],
};

export const api4wingsResponse_multi_dataset: I4wingsAPIResponse = {
  total: 3,
  limit: null,
  offset: null,
  nextOffset: null,
  metadata: {},
  entries: [
    {
      'public-global-sar-presence:v3.0': [
        {
          callsign: 'V2OW7',
          dataset: 'public-global-vessel-identity:v3.0',
          date: '2025-12-04 16:00',
          detections: 1,
          entryTimestamp: '2025-12-04T16:53:26Z',
          exitTimestamp: '2025-12-04T16:53:26Z',
          firstTransmissionDate: '2012-01-02T16:28:28Z',
          flag: 'ATG',
          geartype: 'CARGO',
          imo: '9287807',
          lastTransmissionDate: '2026-02-25T23:56:08Z',
          lat: 55.22,
          lon: 14.40999984741211,
          mmsi: '305773000',
          shipName: 'VOHBURG',
          vesselId: '369fc1e02-2678-b669-af58-b2f3ae66a515',
          vesselType: 'CARGO',
        },
        {
          callsign: '5BNV4',
          dataset: 'public-global-vessel-identity:v3.0',
          date: '2025-12-06 05:00',
          detections: 1,
          entryTimestamp: '2025-12-06T05:25:03Z',
          exitTimestamp: '2025-12-06T05:25:03Z',
          firstTransmissionDate: '2016-08-30T13:30:09Z',
          flag: 'CYP',
          geartype: 'CARGO',
          imo: '9552044',
          lastTransmissionDate: '2026-02-25T23:58:10Z',
          lat: 55.09,
          lon: 14.09000015258789,
          mmsi: '212584000',
          shipName: 'MIRAMAR',
          vesselId: 'afdb60bd6-6d97-eb03-ef84-c1e3b26c10cc',
          vesselType: 'CARGO',
        },
        {
          callsign: 'PBDI',
          dataset: 'public-global-vessel-identity:v3.0',
          date: '2025-12-06 05:00',
          detections: 1,
          entryTimestamp: '2025-12-06T05:25:03Z',
          exitTimestamp: '2025-12-06T05:25:03Z',
          firstTransmissionDate: '2025-10-14T12:26:31Z',
          flag: 'NLD',
          geartype: 'CARGO',
          imo: '1046104',
          lastTransmissionDate: '2026-02-25T23:59:59Z',
          lat: 55.24,
          lon: 14.329999923706055,
          mmsi: '244067000',
          shipName: 'BALTIC SAIL',
          vesselId: '43c5ec9e1-1026-c273-d280-f0c34b520f53',
          vesselType: 'CARGO',
        },
        {
          callsign: 'DFXF',
          dataset: 'public-global-vessel-identity:v3.0',
          date: '2025-12-04 16:00',
          detections: 1,
          entryTimestamp: '2025-12-04T16:53:26Z',
          exitTimestamp: '2025-12-04T16:53:26Z',
          firstTransmissionDate: '2014-07-30T09:19:45Z',
          flag: 'DEU',
          geartype: 'CARGO',
          imo: '9196254',
          lastTransmissionDate: '2026-02-25T23:59:39Z',
          lat: 55.16,
          lon: 14.170000076293945,
          mmsi: '218797000',
          shipName: 'VAERMLAND',
          vesselId: 'b867aac71-181e-e3fb-ef3b-1267b81caedc',
          vesselType: 'CARGO',
        },
        {
          callsign: 'OURV2',
          dataset: 'public-global-vessel-identity:v3.0',
          date: '2025-12-06 05:00',
          detections: 1,
          entryTimestamp: '2025-12-06T05:25:03Z',
          exitTimestamp: '2025-12-06T05:25:03Z',
          firstTransmissionDate: '2012-01-01T00:03:55Z',
          flag: 'DNK',
          geartype: 'CARGO',
          imo: '8104565',
          lastTransmissionDate: '2026-02-25T23:59:41Z',
          lat: 55.23,
          lon: 14.1899995803833,
          mmsi: '219002392',
          shipName: 'AMANDA',
          vesselId: 'df3528985-5287-a35b-89a0-b2e9c1e6db13',
          vesselType: 'CARGO',
        },
        {
          callsign: '5LYK8',
          dataset: 'public-global-vessel-identity:v3.0',
          date: '2025-12-04 16:00',
          detections: 1,
          entryTimestamp: '2025-12-04T16:53:26Z',
          exitTimestamp: '2025-12-04T16:53:26Z',
          firstTransmissionDate: '2025-10-31T03:06:26Z',
          flag: 'LBR',
          geartype: 'OTHER',
          imo: '9312860',
          lastTransmissionDate: '2026-02-25T23:59:56Z',
          lat: 55.15,
          lon: 14.220000267028809,
          mmsi: '636025638',
          shipName: 'IONIAN SUN',
          vesselId: 'ee20f2f54-4c41-ce6c-ce45-fa136fb6e2cc',
          vesselType: 'OTHER',
        },
        {
          callsign: '8PXO8',
          dataset: 'public-global-vessel-identity:v3.0',
          date: '2025-12-04 16:00',
          detections: 1,
          entryTimestamp: '2025-12-04T16:53:26Z',
          exitTimestamp: '2025-12-04T16:53:26Z',
          firstTransmissionDate: '2024-03-20T02:01:54Z',
          flag: 'BRB',
          geartype: 'OTHER',
          imo: '9298662',
          lastTransmissionDate: '2026-02-25T23:59:42Z',
          lat: 55.16,
          lon: 14.109999656677246,
          mmsi: '314842000',
          shipName: 'MULE',
          vesselId: 'de435d0f3-306c-2869-d119-fff03a975f8f',
          vesselType: 'OTHER',
        },
        {
          callsign: 'OXPQ2',
          dataset: 'public-global-vessel-identity:v3.0',
          date: '2025-12-04 16:00',
          detections: 1,
          entryTimestamp: '2025-12-04T16:53:26Z',
          exitTimestamp: '2025-12-04T16:53:26Z',
          firstTransmissionDate: '2018-05-17T13:01:39Z',
          flag: 'DNK',
          geartype: 'PASSENGER',
          imo: '9812107',
          lastTransmissionDate: '2026-02-25T23:58:01Z',
          lat: 55.19,
          lon: 14.239999771118164,
          mmsi: '219026000',
          shipName: 'HAMMERSHUS',
          vesselId: '2cb75b670-08a6-fc17-8fd9-5d9d10a0fbdf',
          vesselType: 'PASSENGER',
        },
        {
          callsign: '',
          dataset: '',
          date: '2025-12-04 16:00',
          detections: 1,
          entryTimestamp: '2025-12-04T16:53:26Z',
          exitTimestamp: '2025-12-04T16:53:26Z',
          firstTransmissionDate: '',
          flag: '',
          geartype: '',
          imo: '',
          lastTransmissionDate: '',
          lat: 55.2,
          lon: 14.210000038146973,
          mmsi: '',
          shipName: '',
          vesselId: '',
          vesselType: '',
        },
        {
          callsign: '5BRS5',
          dataset: 'public-global-vessel-identity:v3.0',
          date: '2025-12-06 05:00',
          detections: 1,
          entryTimestamp: '2025-12-06T05:25:03Z',
          exitTimestamp: '2025-12-06T05:25:03Z',
          firstTransmissionDate: '2021-02-02T09:18:42Z',
          flag: 'CYP',
          geartype: 'CARGO',
          imo: '9213703',
          lastTransmissionDate: '2026-02-25T23:31:26Z',
          lat: 55.21,
          lon: 14.40999984741211,
          mmsi: '209982000',
          shipName: 'NINA B',
          vesselId: 'e4a2ba1ec-c786-5395-6e67-1fb8e1abe320',
          vesselType: 'CARGO',
        },
        {
          callsign: '8POQ',
          dataset: 'public-global-vessel-identity:v3.0',
          date: '2025-12-06 05:00',
          detections: 1,
          entryTimestamp: '2025-12-06T05:25:03Z',
          exitTimestamp: '2025-12-06T05:25:03Z',
          firstTransmissionDate: '2025-10-25T08:35:19Z',
          flag: 'BRB',
          geartype: 'OTHER',
          imo: '9318096',
          lastTransmissionDate: '2026-02-25T23:59:57Z',
          lat: 55.15,
          lon: 14.300000190734863,
          mmsi: '314001147',
          shipName: 'COSMO SAIL',
          vesselId: 'ddd3f8ebb-b915-bcf6-9d8f-5f463eb99b7f',
          vesselType: 'OTHER',
        },
        {
          callsign: 'OVDN',
          dataset: 'public-global-vessel-identity:v3.0',
          date: '2025-12-04 16:00',
          detections: 1,
          entryTimestamp: '2025-12-04T16:53:26Z',
          exitTimestamp: '2025-12-04T16:53:26Z',
          firstTransmissionDate: '2018-09-13T16:47:49Z',
          flag: 'DNK',
          geartype: 'OTHER',
          imo: '',
          lastTransmissionDate: '2026-02-11T17:57:20Z',
          lat: 55.16,
          lon: 14.680000305175781,
          mmsi: '219000217',
          shipName: 'HDMS SOELOEVEN',
          vesselId: '22c7ca42d-df50-18b5-713a-a81e9115052c',
          vesselType: 'OTHER',
        },
        {
          callsign: 'YDJY3',
          dataset: 'public-global-vessel-identity:v3.0',
          date: '2025-12-06 05:00',
          detections: 1,
          entryTimestamp: '2025-12-06T05:25:03Z',
          exitTimestamp: '2025-12-06T05:25:03Z',
          firstTransmissionDate: '2024-05-08T11:07:06Z',
          flag: 'IDN',
          geartype: 'OTHER',
          imo: '9388730',
          lastTransmissionDate: '2026-02-25T23:59:14Z',
          lat: 55.23,
          lon: 14.289999961853027,
          mmsi: '525121076',
          shipName: 'SAVIR TIGER',
          vesselId: '403dc1002-210e-de4b-331d-b4012153dfa4',
          vesselType: 'OTHER',
        },
      ],
      'public-global-presence:v3.0': [
        {
          callsign: 'V4PB6',
          dataset: 'public-global-vessel-identity:v3.0',
          date: '2025-04-01 08:00',
          entryTimestamp: '2025-04-01T00:00:00Z',
          exitTimestamp: '2025-04-01T23:00:00Z',
          firstTransmissionDate: '2025-01-03T02:17:07Z',
          flag: 'KNA',
          geartype: 'CARGO',
          hours: 1,
          imo: '9184938',
          lastTransmissionDate: '2026-05-02T22:36:12Z',
          lat: 28.8,
          lon: 48.529998779296875,
          mmsi: '341036002',
          shipName: 'PORTLAND IV',
          vesselId: 'b7285525c-ca3b-63c2-ddce-1f50cc10ed89',
          vesselType: 'CARGO',
        },
        {
          callsign: '1165',
          dataset: 'public-global-vessel-identity:v3.0',
          date: '2025-04-01 09:00',
          entryTimestamp: '2025-04-01T00:00:00Z',
          exitTimestamp: '2025-04-01T23:00:00Z',
          firstTransmissionDate: '2025-03-27T07:33:01Z',
          flag: 'KWT',
          geartype: 'PASSENGER',
          hours: 1,
          imo: '',
          lastTransmissionDate: '2026-04-24T08:46:23Z',
          lat: 29.39,
          lon: 47.97999954223633,
          mmsi: '447000887',
          shipName: '',
          vesselId: 'd08315d80-08c6-8f05-46bf-ba1218aac1b8',
          vesselType: 'PASSENGER',
        },
        {
          callsign: '5IM689',
          dataset: 'public-global-vessel-identity:v3.0',
          date: '2025-04-01 07:00',
          entryTimestamp: '2025-04-01T00:00:00Z',
          exitTimestamp: '2025-04-01T23:00:00Z',
          firstTransmissionDate: '2022-12-24T12:02:28Z',
          flag: 'TZA',
          geartype: 'OTHER',
          hours: 1,
          imo: '7930527',
          lastTransmissionDate: '2025-11-03T10:45:16Z',
          lat: 28.97,
          lon: 48.310001373291016,
          mmsi: '677058900',
          shipName: 'VIYANA',
          vesselId: '7372598e8-8aaf-daf5-c9dc-4817d87ed0d1',
          vesselType: 'OTHER',
        },
        {
          callsign: 'KEAP',
          dataset: 'public-global-vessel-identity:v3.0',
          date: '2025-04-01 06:00',
          entryTimestamp: '2025-04-01T00:00:00Z',
          exitTimestamp: '2025-04-01T23:00:00Z',
          firstTransmissionDate: '2023-05-22T08:14:21Z',
          flag: 'USA',
          geartype: 'OTHER',
          hours: 1,
          imo: '9760603',
          lastTransmissionDate: '2026-03-02T02:54:20Z',
          lat: 29.35,
          lon: 47.91999816894531,
          mmsi: '369558000',
          shipName: 'APL EAGLE',
          vesselId: '9bac65921-1ea3-1e1c-e84b-65970971e365',
          vesselType: 'OTHER',
        },
        {
          callsign: '1175',
          dataset: 'public-global-vessel-identity:v3.0',
          date: '2025-04-01 18:00',
          entryTimestamp: '2025-04-01T00:00:00Z',
          exitTimestamp: '2025-04-01T23:00:00Z',
          firstTransmissionDate: '2024-10-25T10:26:08Z',
          flag: 'KWT',
          geartype: 'OTHER',
          hours: 1,
          imo: '',
          lastTransmissionDate: '2025-12-07T19:55:08Z',
          lat: 29.37,
          lon: 48.0099983215332,
          mmsi: '447000934',
          shipName: '',
          vesselId: 'e0b94a73d-dbdd-24ad-cb2f-f6a8eec67558',
          vesselType: 'OTHER',
        },
        {
          callsign: 'D8SH',
          dataset: 'public-global-vessel-identity:v3.0',
          date: '2025-04-01 21:00',
          entryTimestamp: '2025-04-01T00:00:00Z',
          exitTimestamp: '2025-04-01T23:00:00Z',
          firstTransmissionDate: '2020-10-08T02:38:59Z',
          flag: 'KOR',
          geartype: 'CARGO',
          hours: 1,
          imo: '9392810',
          lastTransmissionDate: '2026-05-02T00:28:33Z',
          lat: 28.85,
          lon: 48.5,
          mmsi: '441265000',
          shipName: 'SN OPAL',
          vesselId: '6229e67bb-b0ec-d13f-1709-dda48c143de6',
          vesselType: 'CARGO',
        },
        {
          callsign: '9KGE',
          dataset: 'public-global-vessel-identity:v3.0',
          date: '2025-04-01 01:00',
          entryTimestamp: '2025-04-01T00:00:00Z',
          exitTimestamp: '2025-04-01T23:00:00Z',
          firstTransmissionDate: '2015-03-07T13:04:56Z',
          flag: 'KWT',
          geartype: 'OTHER',
          hours: 1,
          imo: '9675987',
          lastTransmissionDate: '2026-04-27T10:21:40Z',
          lat: 29.06,
          lon: 48.150001525878906,
          mmsi: '447205000',
          shipName: 'KOC SHAHEM',
          vesselId: 'ed769f785-54b2-68d1-521a-7d17c2b93471',
          vesselType: 'OTHER',
        },
        {
          callsign: 'HSRN',
          dataset: 'public-global-vessel-identity:v3.0',
          date: '2025-04-01 02:00',
          entryTimestamp: '2025-04-01T00:00:00Z',
          exitTimestamp: '2025-04-01T06:00:00Z',
          firstTransmissionDate: '2012-01-01T01:01:39Z',
          flag: 'THA',
          geartype: 'CARGO',
          hours: 1,
          imo: '9302906',
          lastTransmissionDate: '2026-01-14T09:14:50Z',
          lat: 29.1,
          lon: 48.41999816894531,
          mmsi: '567053000',
          shipName: 'NALINEE NAREE',
          vesselId: '3aee42600-0dda-12cb-fcc1-70ccbe5ff482',
          vesselType: 'CARGO',
        },
        {
          callsign: '9KLX',
          dataset: 'public-global-vessel-identity:v3.0',
          date: '2025-04-01 17:00',
          entryTimestamp: '2025-04-01T00:00:00Z',
          exitTimestamp: '2025-04-01T23:00:00Z',
          firstTransmissionDate: '2022-08-04T10:40:59Z',
          flag: 'KWT',
          geartype: 'OTHER',
          hours: 1,
          imo: '9953626',
          lastTransmissionDate: '2026-03-11T00:31:14Z',
          lat: 29.35,
          lon: 47.91999816894531,
          mmsi: '447328000',
          shipName: 'GHANIM 1',
          vesselId: '82b134e71-1aa9-81a7-04d2-04d0140208ea',
          vesselType: 'OTHER',
        },
        {
          callsign: '9KVL6',
          dataset: 'public-global-vessel-identity:v3.0',
          date: '2025-04-01 03:00',
          entryTimestamp: '2025-04-01T00:00:00Z',
          exitTimestamp: '2025-04-01T22:00:00Z',
          firstTransmissionDate: '2024-12-01T10:13:24Z',
          flag: 'KWT',
          geartype: 'PASSENGER',
          hours: 1,
          imo: '',
          lastTransmissionDate: '2026-05-06T23:58:09Z',
          lat: 29.34,
          lon: 48.04999923706055,
          mmsi: '447004721',
          shipName: '447004721',
          vesselId: '28d84a347-79a3-bf32-d907-5758c899b4e8',
          vesselType: 'PASSENGER',
        },
        {
          callsign: '9KBK2',
          dataset: 'public-global-vessel-identity:v3.0',
          date: '2025-04-01 11:00',
          entryTimestamp: '2025-04-01T04:00:00Z',
          exitTimestamp: '2025-04-01T13:00:00Z',
          firstTransmissionDate: '2024-08-17T19:04:06Z',
          flag: 'KWT',
          geartype: 'PASSENGER',
          hours: 1,
          imo: '',
          lastTransmissionDate: '2025-07-12T14:14:31Z',
          lat: 28.8,
          lon: 48.75,
          mmsi: '447000082',
          shipName: '',
          vesselId: '5f2d9dccf-f411-2465-f5b7-b57d25ded319',
          vesselType: 'PASSENGER',
        },
      ],
      'public-global-fishing-effort:v3.0': [
        {
          callsign: '9KXJ',
          dataset: 'public-global-vessel-identity:v3.0',
          date: '2025-04-01 23:00',
          entryTimestamp: '2025-04-01T00:00:00Z',
          exitTimestamp: '2025-04-01T23:00:00Z',
          firstTransmissionDate: '2024-07-16T05:36:38Z',
          flag: 'KWT',
          geartype: 'FISHING',
          hours: 1.0002777777777778,
          imo: '',
          lastTransmissionDate: '2026-05-05T13:28:45Z',
          lat: 29.24,
          lon: 48.439998626708984,
          mmsi: '447595000',
          shipName: '',
          vesselId: '4c90d1f72-2ab7-11c2-5f34-b52e99bf9da0',
          vesselType: 'FISHING',
        },
        {
          callsign: '9KCY',
          dataset: 'public-global-vessel-identity:v3.0',
          date: '2025-04-01 04:00',
          entryTimestamp: '2025-04-01T00:00:00Z',
          exitTimestamp: '2025-04-01T23:00:00Z',
          firstTransmissionDate: '2024-07-10T17:55:24Z',
          flag: 'KWT',
          geartype: 'TRAWLERS',
          hours: 0.04972222222222222,
          imo: '',
          lastTransmissionDate: '2026-01-15T14:47:25Z',
          lat: 29.14,
          lon: 48.88999938964844,
          mmsi: '447103000',
          shipName: '',
          vesselId: 'edc2f0234-4401-a13f-68b4-5f85aad0c579',
          vesselType: 'FISHING',
        },
        {
          callsign: '9KAG8',
          dataset: 'public-global-vessel-identity:v3.0',
          date: '2025-04-01 04:00',
          entryTimestamp: '2025-04-01T00:00:00Z',
          exitTimestamp: '2025-04-01T23:00:00Z',
          firstTransmissionDate: '2024-07-15T17:29:56Z',
          flag: 'KWT',
          geartype: 'TRAWLERS',
          hours: 1.0038888888888888,
          imo: '',
          lastTransmissionDate: '2026-05-05T03:46:41Z',
          lat: 29.02,
          lon: 48.849998474121094,
          mmsi: '447717000',
          shipName: '',
          vesselId: '9aa782fe9-9a6c-352b-4f6a-80b9b9cfe69e',
          vesselType: 'FISHING',
        },
        {
          callsign: '9KOJ',
          dataset: 'public-global-vessel-identity:v3.0',
          date: '2025-04-01 08:00',
          entryTimestamp: '2025-04-01T01:00:00Z',
          exitTimestamp: '2025-04-01T14:00:00Z',
          firstTransmissionDate: '2024-09-07T02:55:05Z',
          flag: 'KWT',
          geartype: 'SET_GILLNETS',
          hours: 0.5991666666666666,
          imo: '',
          lastTransmissionDate: '2026-05-06T23:30:42Z',
          lat: 29.7,
          lon: 48.43000030517578,
          mmsi: '447386000',
          shipName: '',
          vesselId: 'f0cffa5a1-1dd4-9c3a-274e-7ba65be3c853',
          vesselType: 'FISHING',
        },
        {
          callsign: '9KCY',
          dataset: 'public-global-vessel-identity:v3.0',
          date: '2025-04-01 11:00',
          entryTimestamp: '2025-04-01T00:00:00Z',
          exitTimestamp: '2025-04-01T23:00:00Z',
          firstTransmissionDate: '2024-07-10T17:55:24Z',
          flag: 'KWT',
          geartype: 'TRAWLERS',
          hours: 0.9011111111111112,
          imo: '',
          lastTransmissionDate: '2026-01-15T14:47:25Z',
          lat: 28.92,
          lon: 48.9900016784668,
          mmsi: '447103000',
          shipName: '',
          vesselId: 'edc2f0234-4401-a13f-68b4-5f85aad0c579',
          vesselType: 'FISHING',
        },
        {
          callsign: '',
          dataset: 'public-global-vessel-identity:v3.0',
          date: '2025-04-01 02:00',
          entryTimestamp: '2025-04-01T00:00:00Z',
          exitTimestamp: '2025-04-01T05:00:00Z',
          firstTransmissionDate: '2024-07-26T13:25:09Z',
          flag: 'KWT',
          geartype: 'FISHING',
          hours: 1.0505555555555555,
          imo: '',
          lastTransmissionDate: '2025-04-06T02:31:20Z',
          lat: 29.49,
          lon: 48.66999816894531,
          mmsi: '447621000',
          shipName: '447621000',
          vesselId: '42d5724c1-1861-630e-3878-a3069ffa1948',
          vesselType: 'FISHING',
        },
        {
          callsign: '',
          dataset: 'public-global-vessel-identity:v3.0',
          date: '2025-04-01 12:00',
          entryTimestamp: '2025-04-01T02:00:00Z',
          exitTimestamp: '2025-04-01T23:00:00Z',
          firstTransmissionDate: '2024-08-09T19:21:51Z',
          flag: 'KWT',
          geartype: 'FISHING',
          hours: 0.9994444444444445,
          imo: '',
          lastTransmissionDate: '2026-05-05T12:24:01Z',
          lat: 29.23,
          lon: 48.209999084472656,
          mmsi: '447752000',
          shipName: '447752000',
          vesselId: 'ea177c886-68c0-d898-08d4-2bb6659138ea',
          vesselType: 'FISHING',
        },
        {
          callsign: '9KNZ',
          dataset: 'public-global-vessel-identity:v3.0',
          date: '2025-04-01 05:00',
          entryTimestamp: '2025-04-01T01:00:00Z',
          exitTimestamp: '2025-04-01T23:00:00Z',
          firstTransmissionDate: '2024-08-24T08:05:20Z',
          flag: 'KWT',
          geartype: 'SET_GILLNETS',
          hours: 0.7847222222222221,
          imo: '',
          lastTransmissionDate: '2026-02-05T06:02:52Z',
          lat: 29.56,
          lon: 48.380001068115234,
          mmsi: '447376000',
          shipName: '',
          vesselId: '6ea1dec77-7ad3-a9a9-feb4-ccacacc8bfbd',
          vesselType: 'FISHING',
        },
        {
          callsign: '9KQF',
          dataset: 'public-global-vessel-identity:v3.0',
          date: '2025-04-01 08:00',
          entryTimestamp: '2025-04-01T00:00:00Z',
          exitTimestamp: '2025-04-01T23:00:00Z',
          firstTransmissionDate: '2024-07-31T13:57:22Z',
          flag: 'KWT',
          geartype: 'FISHING',
          hours: 0.8486111111111111,
          imo: '',
          lastTransmissionDate: '2026-04-10T02:00:13Z',
          lat: 29.49,
          lon: 48.61000061035156,
          mmsi: '447443000',
          shipName: 'GHANI 7',
          vesselId: '5720547ff-f558-b049-51c5-9bd87ae52a22',
          vesselType: 'FISHING',
        },
        {
          callsign: '9KAD9',
          dataset: 'public-global-vessel-identity:v3.0',
          date: '2025-04-01 03:00',
          entryTimestamp: '2025-04-01T00:00:00Z',
          exitTimestamp: '2025-04-01T23:00:00Z',
          firstTransmissionDate: '2024-07-14T08:04:20Z',
          flag: 'KWT',
          geartype: 'TRAWLERS',
          hours: 0.9991666666666666,
          imo: '',
          lastTransmissionDate: '2026-02-18T11:13:29Z',
          lat: 29.1,
          lon: 48.880001068115234,
          mmsi: '447694000',
          shipName: '447694000',
          vesselId: '678fed172-26d1-793d-96c9-a7ac12b58bae',
          vesselType: 'FISHING',
        },
        {
          callsign: '9KQX',
          dataset: 'public-global-vessel-identity:v3.0',
          date: '2025-04-01 18:00',
          entryTimestamp: '2025-04-01T00:00:00Z',
          exitTimestamp: '2025-04-01T23:00:00Z',
          firstTransmissionDate: '2024-08-06T13:50:47Z',
          flag: 'KWT',
          geartype: 'FISHING',
          hours: 1.002777777777778,
          imo: '',
          lastTransmissionDate: '2026-04-11T12:13:15Z',
          lat: 29.13,
          lon: 48.4900016784668,
          mmsi: '447461000',
          shipName: 'ALDAHAR',
          vesselId: 'd6a428e6d-df99-262a-f436-d8d55e728795',
          vesselType: 'FISHING',
        },
        {
          callsign: '9KOU',
          dataset: 'public-global-vessel-identity:v3.0',
          date: '2025-04-01 00:00',
          entryTimestamp: '2025-04-01T00:00:00Z',
          exitTimestamp: '2025-04-01T11:00:00Z',
          firstTransmissionDate: '2024-09-14T01:47:04Z',
          flag: 'KWT',
          geartype: 'SET_GILLNETS',
          hours: 1.0069444444444449,
          imo: '',
          lastTransmissionDate: '2026-05-06T22:47:04Z',
          lat: 29.65,
          lon: 48.36000061035156,
          mmsi: '447397000',
          shipName: '',
          vesselId: '78be39db9-99b0-de2b-26df-cf3af8057e8d',
          vesselType: 'FISHING',
        },
        {
          callsign: '9KAF4',
          dataset: 'public-global-vessel-identity:v3.0',
          date: '2025-04-01 20:00',
          entryTimestamp: '2025-04-01T17:00:00Z',
          exitTimestamp: '2025-04-01T23:00:00Z',
          firstTransmissionDate: '2024-07-20T15:32:54Z',
          flag: 'KWT',
          geartype: 'TRAWLERS',
          hours: 1.2502777777777778,
          imo: '',
          lastTransmissionDate: '2025-11-09T10:29:19Z',
          lat: 29.35,
          lon: 48.90999984741211,
          mmsi: '447705000',
          shipName: '',
          vesselId: '198df23fc-c14e-b3bf-1712-71e5887bbe4a',
          vesselType: 'FISHING',
        },
        {
          callsign: '9KAG7',
          dataset: 'public-global-vessel-identity:v3.0',
          date: '2025-04-01 13:00',
          entryTimestamp: '2025-04-01T00:00:00Z',
          exitTimestamp: '2025-04-01T23:00:00Z',
          firstTransmissionDate: '2024-07-15T17:51:25Z',
          flag: 'KWT',
          geartype: 'TRAWLERS',
          hours: 1.5991666666666666,
          imo: '',
          lastTransmissionDate: '2026-05-05T03:55:18Z',
          lat: 29.09,
          lon: 48.880001068115234,
          mmsi: '447716000',
          shipName: '',
          vesselId: '36350b33b-b878-fc79-8ee3-2ce18c5bbe95',
          vesselType: 'FISHING',
        },
      ],
    },
  ],
};

export const api4wingsResponse_mixed_accepted_rejected: I4wingsAPIResponse = {
  total: 1,
  limit: null,
  offset: null,
  nextOffset: null,
  metadata: {},
  entries: [
    {
      'public-global-sar-presence:v3.0': [
        api4wingsResponse_bad_vessel_type.entries[0][
          'public-global-sar-presence:v3.0'
        ][0],
        api4wingsResponse_bad_coordinates.entries[0][
          'public-global-sar-presence:v3.0'
        ][0],
        api4wingsResponse_bad_date.entries[0][
          'public-global-sar-presence:v3.0'
        ][0],
        api4wingsResponse_bad_multi.entries[0][
          'public-global-sar-presence:v3.0'
        ][0],
        api4wingsResponse.entries[0]['public-global-sar-presence:v3.0'][0],
        api4wingsResponse.entries[0]['public-global-sar-presence:v3.0'][1],
        api4wingsResponse.entries[0]['public-global-sar-presence:v3.0'][2],
      ],
    },
  ],
};

export const api4wingsEntry_matched: I4wingsEntry = {
  callsign: 'V2OW7',
  dataset: 'public-global-vessel-identity:v3.0',
  date: '2025-12-04 16:00',
  detections: 1,
  entryTimestamp: '2025-12-04T16:53:26Z',
  exitTimestamp: '2025-12-04T16:53:26Z',
  firstTransmissionDate: '2012-01-02T16:28:28Z',
  flag: 'ATG',
  geartype: 'CARGO',
  imo: '9287807',
  lastTransmissionDate: '2026-02-25T23:56:08Z',
  lat: 55.22,
  lon: 14.40999984741211,
  mmsi: '305773000',
  shipName: 'VOHBURG',
  vesselId: '369fc1e02-2678-b669-af58-b2f3ae66a515',
  vesselType: 'CARGO',
};

export const api4wingsEntry_matched_detections_2: I4wingsEntry = {
  ...api4wingsEntry_matched,
  detections: 2,
};

export const api4wingsEntry_matched_detections_5: I4wingsEntry = {
  ...api4wingsEntry_matched,
  detections: 5,
};

export const api4wingsEntry_unmatched: I4wingsEntry = {
  callsign: '',
  dataset: '',
  date: '2025-12-04 16:00',
  detections: 1,
  entryTimestamp: '2025-12-04T16:53:26Z',
  exitTimestamp: '2025-12-04T16:53:26Z',
  firstTransmissionDate: '',
  flag: '',
  geartype: '',
  imo: '',
  lastTransmissionDate: '',
  lat: 55.2,
  lon: 14.210000038146973,
  mmsi: '',
  shipName: '',
  vesselId: '',
  vesselType: '',
};

export const api4wingsEntry_unmatched_detections_2: I4wingsEntry = {
  ...api4wingsEntry_unmatched,
  detections: 2,
};

export const api4wingsEntry_unmatched_detections_5: I4wingsEntry = {
  ...api4wingsEntry_unmatched,
  detections: 5,
};

export const api4wingsEntry_noisy: I4wingsEntry = {
  callsign: '',
  dataset: '',
  date: '2017-01-01 22:00',
  detections: 1,
  entryTimestamp: '2017-01-01T22:33:41Z',
  exitTimestamp: '2017-01-01T22:33:41Z',
  firstTransmissionDate: '',
  flag: '',
  geartype: '',
  imo: '',
  lastTransmissionDate: '',
  lat: -6.09,
  lon: 106.89,
  mmsi: '',
  shipName: '',
  vesselId: '74934b786-6f6f-d027-c06f-bf814d7da7f3',
  vesselType: '',
};

export const api4wingsEntry_noisy_high_detections: I4wingsEntry = {
  callsign: '',
  dataset: '',
  date: '2017-01-01 22:00',
  detections: 2,
  entryTimestamp: '2017-01-01T22:33:41Z',
  exitTimestamp: '2017-01-01T22:33:41Z',
  firstTransmissionDate: '',
  flag: '',
  geartype: '',
  imo: '',
  lastTransmissionDate: '',
  lat: -6.09,
  lon: 106.89,
  mmsi: '',
  shipName: '',
  vesselId: '74934b786-6f6f-d027-c06f-bf814d7da7f3',
  vesselType: '',
};

export const api4wingsEntry_missing_date: I4wingsEntry = {
  callsign: 'V2OW7',
  dataset: 'public-global-vessel-identity:v3.0',
  date: '',
  detections: 1,
  entryTimestamp: '2025-12-04T16:53:26Z',
  exitTimestamp: '2025-12-04T16:53:26Z',
  firstTransmissionDate: '2012-01-02T16:28:28Z',
  flag: 'ATG',
  geartype: 'CARGO',
  imo: '9287807',
  lastTransmissionDate: '2026-02-25T23:56:08Z',
  lat: 55.22,
  lon: 14.40999984741211,
  mmsi: '305773000',
  shipName: 'VOHBURG',
  vesselId: '369fc1e02-2678-b669-af58-b2f3ae66a515',
  vesselType: 'CARGO',
};

//@ts-ignore
export const api4wingsEntry_missing_coordinates: I4wingsEntry = {
  callsign: 'V2OW7',
  dataset: 'public-global-vessel-identity:v3.0',
  date: '2025-12-04 16:00',
  detections: 1,
  entryTimestamp: '2025-12-04T16:53:26Z',
  exitTimestamp: '2025-12-04T16:53:26Z',
  firstTransmissionDate: '2012-01-02T16:28:28Z',
  flag: 'ATG',
  geartype: 'CARGO',
  imo: '9287807',
  lastTransmissionDate: '2026-02-25T23:56:08Z',
  /* lat: 55.22,
  lon: 14.40999984741211, */
  mmsi: '305773000',
  shipName: 'VOHBURG',
  vesselId: '369fc1e02-2678-b669-af58-b2f3ae66a515',
  vesselType: 'CARGO',
};

export const apiEventResponse_no_entry: IEventAPIResponse<IPortVisitEvent> = {
  metadata: {
    datasets: ['public-global-port-visits-events:v3.0'],
    vessels: ['369fc1e02-2678-b669-af58-b2f3ae66a515'],
    dateRange: {
      from: '2025-12-04T00:00:00Z',
      to: '2025-12-06T23:59:59Z',
    },
    encounterTypes: [],
    geometry: {
      type: 'FeatureCollection',
      features: [
        {
          type: 'Feature',
          geometry: {
            type: 'Polygon',
            coordinates: [
              [
                [14.11, 55.26],
                [14.68, 55.27],
                [14.69, 55.11],
                [14.09, 55.08],
                [14.11, 55.26],
              ],
            ],
          },
        },
      ],
    },
  },
  limit: 2,
  offset: 0,
  nextOffset: null,
  total: 0,
  entries: [],
};

export const apiEventResponse_with_entry: IEventAPIResponse<IPortVisitEvent> = {
  metadata: {
    datasets: ['public-global-port-visits-events:v3.0'],
    vessels: ['22c7ca42d-df50-18b5-713a-a81e9115052c'],
    dateRange: {
      from: '2025-12-04T00:00:00Z',
      to: '2025-12-06T23:59:59Z',
    },
    encounterTypes: [],
    geometry: {
      type: 'FeatureCollection',
      features: [
        {
          type: 'Feature',
          geometry: {
            type: 'Polygon',
            coordinates: [
              [
                [14.11, 55.26],
                [14.68, 55.27],
                [14.69, 55.11],
                [14.09, 55.08],
                [14.11, 55.26],
              ],
            ],
          },
        },
      ],
    },
  },
  limit: 2,
  offset: 0,
  nextOffset: null,
  total: 1,
  entries: [
    {
      start: '2025-12-05T12:06:41.000Z',
      end: '2025-12-06T12:08:08.000Z',
      id: 'efc1703a86346486d884cbd5fc5aab74',
      type: EEventType.port_visit,
      position: {
        lat: 55.1155,
        lon: 14.6758,
      },
      regions: {
        mpa: ['555790698', '555543143', '555522525'],
        eez: ['5674'],
        rfmo: ['NASCO', 'ACAP', 'NAMMCO', 'ICCAT', 'IWC', 'ICES'],
        fao: ['27.3.d.24', '27.3.d', '27.3', '27'],
        majorFao: ['27'],
        eez12Nm: ['5674'],
        highSeas: [],
        mpaNoTakePartial: [],
        mpaNoTake: [],
      },
      boundingBox: [
        14.693334446163684, 55.105045722833694, 14.693334446163684,
        55.105045722833694,
      ],
      distances: {
        startDistanceFromShoreKm: 0,
        endDistanceFromShoreKm: 0,
        startDistanceFromPortKm: 0,
        endDistanceFromPortKm: 0,
      },
      vessel: {
        id: '22c7ca42d-df50-18b5-713a-a81e9115052c',
        name: 'HDMS SOELOEVEN',
        ssvid: '219000217',
        flag: 'DNK',
        type: 'other',
        nextPort: null,
      },
      port_visit: {
        visitId: '22efc11c118fb011f640e6328d54744f',
        confidence: '4',
        durationHrs: 24.024166666666666,
        startAnchorage: {
          anchorageId: '46551b1d',
          atDock: true,
          distanceFromShoreKm: 0,
          flag: 'DNK',
          id: 'dnk-roenne',
          lat: 55.105045722833694,
          lon: 14.693334446163684,
          name: 'ROENNE',
          topDestination: 'BAY LIGHTER',
        },
        intermediateAnchorage: {
          anchorageId: '46551b1d',
          atDock: true,
          distanceFromShoreKm: 0,
          flag: 'DNK',
          id: 'dnk-roenne',
          lat: 55.105045722833694,
          lon: 14.693334446163684,
          name: 'ROENNE',
          topDestination: 'BAY LIGHTER',
        },
        endAnchorage: {
          anchorageId: '46551b1d',
          atDock: true,
          distanceFromShoreKm: 0,
          flag: 'DNK',
          id: 'dnk-roenne',
          lat: 55.105045722833694,
          lon: 14.693334446163684,
          name: 'ROENNE',
          topDestination: 'BAY LIGHTER',
        },
      },
    },
  ],
};

export const apiEventResponse_with_entry_low_confidence: IEventAPIResponse<IPortVisitEvent> =
  {
    metadata: {
      datasets: ['public-global-port-visits-events:v3.0'],
      vessels: ['22c7ca42d-df50-18b5-713a-a81e9115052c'],
      dateRange: {
        from: '2025-12-04T00:00:00Z',
        to: '2025-12-06T23:59:59Z',
      },
      encounterTypes: [],
      geometry: {
        type: 'FeatureCollection',
        features: [
          {
            type: 'Feature',
            geometry: {
              type: 'Polygon',
              coordinates: [
                [
                  [14.11, 55.26],
                  [14.68, 55.27],
                  [14.69, 55.11],
                  [14.09, 55.08],
                  [14.11, 55.26],
                ],
              ],
            },
          },
        ],
      },
    },
    limit: 2,
    offset: 0,
    nextOffset: null,
    total: 1,
    entries: [
      {
        start: '2025-12-05T12:06:41.000Z',
        end: '2025-12-06T12:08:08.000Z',
        id: 'efc1703a86346486d884cbd5fc5aab74',
        type: EEventType.port_visit,
        position: {
          lat: 55.1155,
          lon: 14.6758,
        },
        regions: {
          mpa: ['555790698', '555543143', '555522525'],
          eez: ['5674'],
          rfmo: ['NASCO', 'ACAP', 'NAMMCO', 'ICCAT', 'IWC', 'ICES'],
          fao: ['27.3.d.24', '27.3.d', '27.3', '27'],
          majorFao: ['27'],
          eez12Nm: ['5674'],
          highSeas: [],
          mpaNoTakePartial: [],
          mpaNoTake: [],
        },
        boundingBox: [
          14.693334446163684, 55.105045722833694, 14.693334446163684,
          55.105045722833694,
        ],
        distances: {
          startDistanceFromShoreKm: 0,
          endDistanceFromShoreKm: 0,
          startDistanceFromPortKm: 0,
          endDistanceFromPortKm: 0,
        },
        vessel: {
          id: '22c7ca42d-df50-18b5-713a-a81e9115052c',
          name: 'HDMS SOELOEVEN',
          ssvid: '219000217',
          flag: 'DNK',
          type: 'other',
          nextPort: null,
        },
        port_visit: {
          visitId: '22efc11c118fb011f640e6328d54744f',
          confidence: '2',
          durationHrs: 24.024166666666666,
          startAnchorage: {
            anchorageId: '46551b1d',
            atDock: true,
            distanceFromShoreKm: 0,
            flag: 'DNK',
            id: 'dnk-roenne',
            lat: 55.105045722833694,
            lon: 14.693334446163684,
            name: 'ROENNE',
            topDestination: 'BAY LIGHTER',
          },
          intermediateAnchorage: {
            anchorageId: '46551b1d',
            atDock: true,
            distanceFromShoreKm: 0,
            flag: 'DNK',
            id: 'dnk-roenne',
            lat: 55.105045722833694,
            lon: 14.693334446163684,
            name: 'ROENNE',
            topDestination: 'BAY LIGHTER',
          },
          endAnchorage: {
            anchorageId: '46551b1d',
            atDock: true,
            distanceFromShoreKm: 0,
            flag: 'DNK',
            id: 'dnk-roenne',
            lat: 55.105045722833694,
            lon: 14.693334446163684,
            name: 'ROENNE',
            topDestination: 'BAY LIGHTER',
          },
        },
      },
    ],
  };

export const api4wingsEntry_fishing: I4wingsEntry = {
  callsign: '9KAF4',
  dataset: 'public-global-vessel-identity:v3.0',
  date: '2025-04-01 17:00',
  entryTimestamp: '2025-04-01T17:00:00Z',
  exitTimestamp: '2025-04-01T23:00:00Z',
  firstTransmissionDate: '2024-07-20T15:32:54Z',
  flag: 'KWT',
  geartype: 'TRAWLERS',
  hours: 1.6472222222222226,
  imo: '',
  lastTransmissionDate: '2025-11-09T10:29:19Z',
  lat: 29.35,
  lon: 48.91999816894531,
  mmsi: '447705000',
  shipName: '',
  vesselId: '198df23fc-c14e-b3bf-1712-71e5887bbe4a',
  vesselType: 'FISHING',
};

export const api4wingsEntry_fishing_2: I4wingsEntry = {
  callsign: '9KAF4',
  dataset: 'public-global-vessel-identity:v3.0',
  date: '2025-04-01 17:00',
  entryTimestamp: '2025-04-01T17:00:00Z',
  exitTimestamp: '2025-04-01T23:00:00Z',
  firstTransmissionDate: '2024-07-20T15:32:54Z',
  flag: 'KWT',
  geartype: 'TRAWLERS',
  hours: 2.6472222222222226,
  imo: '',
  lastTransmissionDate: '2025-11-09T10:29:19Z',
  lat: 29.35,
  lon: 48.91999816894531,
  mmsi: '447705000',
  shipName: '',
  vesselId: '198df23fc-c14e-b3bf-1712-71e5887bbe4a',
  vesselType: 'FISHING',
};

export const api4wingsEntry_fishing_5: I4wingsEntry = {
  callsign: '9KAF4',
  dataset: 'public-global-vessel-identity:v3.0',
  date: '2025-04-01 17:00',
  entryTimestamp: '2025-04-01T17:00:00Z',
  exitTimestamp: '2025-04-01T23:00:00Z',
  firstTransmissionDate: '2024-07-20T15:32:54Z',
  flag: 'KWT',
  geartype: 'TRAWLERS',
  hours: 5.647222,
  imo: '',
  lastTransmissionDate: '2025-11-09T10:29:19Z',
  lat: 29.35,
  lon: 48.91999816894531,
  mmsi: '447705000',
  shipName: '',
  vesselId: '198df23fc-c14e-b3bf-1712-71e5887bbe4a',
  vesselType: 'FISHING',
};

export const api4wingsEntry_ais = {
  callsign: 'CQAB3',
  dataset: 'public-global-vessel-identity:v3.0',
  date: '2025-04-01 22:00',
  entryTimestamp: '2025-04-01T22:00:00Z',
  exitTimestamp: '2025-04-01T22:00:00Z',
  firstTransmissionDate: '2018-02-25T01:16:10Z',
  flag: 'PRT',
  geartype: 'CARGO',
  hours: 1,
  imo: '9314959',
  lastTransmissionDate: '2026-05-04T16:39:37Z',
  lat: 29.86,
  lon: 48.369998931884766,
  mmsi: '255806125',
  shipName: 'AS CLEMENTINA',
  vesselId: 'dd162713e-e5b0-3401-e815-501d9715f42e',
  vesselType: 'CARGO',
};

export const api4wingsEntry_ais_2 = {
  callsign: 'CQAB3',
  dataset: 'public-global-vessel-identity:v3.0',
  date: '2025-04-01 22:00',
  entryTimestamp: '2025-04-01T22:00:00Z',
  exitTimestamp: '2025-04-01T22:00:00Z',
  firstTransmissionDate: '2018-02-25T01:16:10Z',
  flag: 'PRT',
  geartype: 'CARGO',
  hours: 2,
  imo: '9314959',
  lastTransmissionDate: '2026-05-04T16:39:37Z',
  lat: 29.86,
  lon: 48.369998931884766,
  mmsi: '255806125',
  shipName: 'AS CLEMENTINA',
  vesselId: 'dd162713e-e5b0-3401-e815-501d9715f42e',
  vesselType: 'CARGO',
};

export const api4wingsEntry_ais_5 = {
  callsign: 'CQAB3',
  dataset: 'public-global-vessel-identity:v3.0',
  date: '2025-04-01 22:00',
  entryTimestamp: '2025-04-01T22:00:00Z',
  exitTimestamp: '2025-04-01T22:00:00Z',
  firstTransmissionDate: '2018-02-25T01:16:10Z',
  flag: 'PRT',
  geartype: 'CARGO',
  hours: 5,
  imo: '9314959',
  lastTransmissionDate: '2026-05-04T16:39:37Z',
  lat: 29.86,
  lon: 48.369998931884766,
  mmsi: '255806125',
  shipName: 'AS CLEMENTINA',
  vesselId: 'dd162713e-e5b0-3401-e815-501d9715f42e',
  vesselType: 'CARGO',
};

/**
 * Header fixtures for validateViolation tests.
 *
 * Trigger logic (both conditions must be true to fire an error):
 *   Monthly exceeded: monthlyRemaining === 0  AND  monthlyResetDays > 0
 *   Daily exceeded:   dailyRemaining   === 0  AND  dailyResetHours  > 0
 */

// ─── Healthy (no violations) ─────────────────────────────────────────────────

/** Both quotas have remaining capacity and no reset pending. isValid should be true. */
export const HEADERS_HEALTHY: Record<string, string> = {
  'x-ratelimit-daily-limit-requests': '50000',
  'x-ratelimit-monthly-limit-requests': '1500000',
  'x-ratelimit-daily-remaining-requests': '12430',
  'x-ratelimit-monthly-remaining-requests': '842310',
  'x-ratelimit-daily-current-usage': '37570',
  'x-ratelimit-monthly-current-usage': '657690',
  'x-ratelimit-daily-reset-hours': '18',
  'x-ratelimit-monthly-reset-days': '27',
};

/** Remaining > 0 on both, reset counters are 0 — cleanest possible healthy state. */
export const HEADERS_HEALTHY_NO_RESET_PENDING: Record<string, string> = {
  'x-ratelimit-daily-remaining-requests': '50000',
  'x-ratelimit-monthly-remaining-requests': '1500000',
  'x-ratelimit-daily-reset-hours': '0',
  'x-ratelimit-monthly-reset-days': '0',
};

// ─── Daily exhausted ─────────────────────────────────────────────────────────

/** Daily quota fully used and reset is still hours away — triggers DAILY error only. */
export const HEADERS_DAILY_EXHAUSTED: Record<string, string> = {
  'x-ratelimit-daily-remaining-requests': '0',
  'x-ratelimit-monthly-remaining-requests': '842310',
  'x-ratelimit-daily-reset-hours': '6',
  'x-ratelimit-monthly-reset-days': '27',
};

/** Daily remaining is 0 but reset is also 0 — quota just reset, no error expected. */
export const HEADERS_DAILY_REMAINING_ZERO_RESET_ZERO: Record<string, string> = {
  'x-ratelimit-daily-remaining-requests': '0',
  'x-ratelimit-monthly-remaining-requests': '842310',
  'x-ratelimit-daily-reset-hours': '0',
  'x-ratelimit-monthly-reset-days': '27',
};

/** Daily reset hours > 0 but remaining > 0 — no error expected (not exhausted). */
export const HEADERS_DAILY_RESET_HOURS_NONZERO_REMAINING_NONZERO: Record<
  string,
  string
> = {
  'x-ratelimit-daily-remaining-requests': '100',
  'x-ratelimit-monthly-remaining-requests': '842310',
  'x-ratelimit-daily-reset-hours': '18',
  'x-ratelimit-monthly-reset-days': '27',
};

// ─── Monthly exhausted ───────────────────────────────────────────────────────

/** Monthly quota fully used and reset is days away — triggers MONTHLY error only. */
export const HEADERS_MONTHLY_EXHAUSTED: Record<string, string> = {
  'x-ratelimit-daily-remaining-requests': '12430',
  'x-ratelimit-monthly-remaining-requests': '0',
  'x-ratelimit-daily-reset-hours': '18',
  'x-ratelimit-monthly-reset-days': '5',
};

/** Monthly remaining is 0 but reset is also 0 — quota just reset, no error expected. */
export const HEADERS_MONTHLY_REMAINING_ZERO_RESET_ZERO: Record<string, string> =
  {
    'x-ratelimit-daily-remaining-requests': '12430',
    'x-ratelimit-monthly-remaining-requests': '0',
    'x-ratelimit-daily-reset-hours': '18',
    'x-ratelimit-monthly-reset-days': '0',
  };

/** Monthly reset days > 0 but remaining > 0 — no error expected (not exhausted). */
export const HEADERS_MONTHLY_RESET_DAYS_NONZERO_REMAINING_NONZERO: Record<
  string,
  string
> = {
  'x-ratelimit-daily-remaining-requests': '12430',
  'x-ratelimit-monthly-remaining-requests': '1000',
  'x-ratelimit-daily-reset-hours': '18',
  'x-ratelimit-monthly-reset-days': '27',
};

// ─── Both exhausted ──────────────────────────────────────────────────────────

/** Both daily and monthly quotas exhausted with resets pending — triggers both errors. */
export const HEADERS_BOTH_EXHAUSTED: Record<string, string> = {
  'x-ratelimit-daily-remaining-requests': '0',
  'x-ratelimit-monthly-remaining-requests': '0',
  'x-ratelimit-daily-reset-hours': '6',
  'x-ratelimit-monthly-reset-days': '5',
};

// ─── Edge / malformed ────────────────────────────────────────────────────────

/** All rate-limit headers are missing entirely. */
export const HEADERS_MISSING_RATELIMIT_FIELDS: Record<string, string> = {
  'content-type': 'application/json',
};

/** All rate-limit values are empty strings — Number('') === 0. */
export const HEADERS_EMPTY_STRING_VALUES: Record<string, string> = {
  'x-ratelimit-daily-remaining-requests': '',
  'x-ratelimit-monthly-remaining-requests': '',
  'x-ratelimit-daily-reset-hours': '',
  'x-ratelimit-monthly-reset-days': '',
};

/** Rate-limit values are non-numeric strings — Number('abc') === NaN. */
export const HEADERS_NON_NUMERIC_VALUES: Record<string, string> = {
  'x-ratelimit-daily-remaining-requests': 'abc',
  'x-ratelimit-monthly-remaining-requests': 'xyz',
  'x-ratelimit-daily-reset-hours': 'foo',
  'x-ratelimit-monthly-reset-days': 'bar',
};
