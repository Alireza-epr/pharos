...
import { validateBodyParams, validateQueryParams, validateViolation } from '../src/helpers/utils/validationUtils';
import {
  invalidBody_geojson,
...
import { HEADERS_BOTH_EXHAUSTED, HEADERS_DAILY_EXHAUSTED, HEADERS_DAILY_REMAINING_ZERO_RESET_ZERO, HEADERS_DAILY_RESET_HOURS_NONZERO_REMAINING_NONZERO, HEADERS_EMPTY_STRING_VALUES, HEADERS_HEALTHY, HEADERS_HEALTHY_NO_RESET_PENDING, HEADERS_MISSING_RATELIMIT_FIELDS, HEADERS_MONTHLY_EXHAUSTED, HEADERS_MONTHLY_REMAINING_ZERO_RESET_ZERO, HEADERS_MONTHLY_RESET_DAYS_NONZERO_REMAINING_NONZERO, HEADERS_NON_NUMERIC_VALUES } from './fixtures/gfwResponse';
import {
...
describe('validateViolation', () => {
  ...
  it('adds_daily_rate_limit_exceeded_error_when_daily_quota_is_exhausted', () => {
    ...
    expect(errorCodes(result)).toContain(EViolationError.DAILY_RATE_LIMIT_EXCEEDED)
  })

  it('does_not_add_monthly_error_when_only_daily_quota_is_exhausted', () => {
    const result = validateViolation(HEADERS_DAILY_EXHAUSTED)

    expect(errorCodes(result)).not.toContain(EViolationError.MONTHLY_RATE_LIMIT_EXCEEDED)
  ...
  it('daily_error_references_correct_header_field', () => {
    ...
    expect(result.errors?.[0].field).toBe('x-ratelimit-daily-remaining-requests')
  })
  ...
  it('does_not_trigger_daily_error_when_remaining_is_0_but_reset_hours_is_also_0', () => {
    ...
    expect(errorCodes(result)).not.toContain(EViolationError.DAILY_RATE_LIMIT_EXCEEDED)
  })
  ...
  it('does_not_trigger_daily_error_when_reset_hours_is_nonzero_but_remaining_is_also_nonzero', () => {
    ...
    const result = validateViolation(HEADERS_DAILY_RESET_HOURS_NONZERO_REMAINING_NONZERO)
  ...
  it('adds_monthly_rate_limit_exceeded_error_when_monthly_quota_is_exhausted', () => {
    ...
    expect(errorCodes(result)).toContain(EViolationError.MONTHLY_RATE_LIMIT_EXCEEDED)
  })
  ...
  it('monthly_error_references_correct_header_field', () => {
    ...
    expect(result.errors?.[0].field).toBe('x-ratelimit-monthly-remaining-requests')
  })
  ...
  it('does_not_trigger_monthly_error_when_reset_days_is_nonzero_but_remaining_is_also_nonzero', () => {
    ...
    const result = validateViolation(HEADERS_MONTHLY_RESET_DAYS_NONZERO_REMAINING_NONZERO)
