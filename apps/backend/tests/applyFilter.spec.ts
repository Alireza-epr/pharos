...
const EVENT_ID = {
    NEAR_COAST:           'cbbf5b2f2035f0eb692ccb99d5b7f2ae1f16a65e9f41a2b3498f8c7e32d0546a', // index 8  – dist 1.33, triage 0.83, bath -22, near_coast
    ...
    LOW_TRIAGE_FIRST:     '97ce71fec35ea516723c7bd7821bf40f9e4d1763c5027afe649da6a5600d4a23', // index 1  – triage 0.53
} as const
...
describe('applyFilter_triage_score', () => {
    ...
    it('returns_only_perfect_score_event_when_range_is_1_to_1', () => {
        const result = applyFilter(events as any, FILTER_TRIAGE_SCORE_RANGE_PERFECT)
    ...
    it('returns_empty_array_when_min_exceeds_max', () => {
        const result = applyFilter(events as any, FILTER_TRIAGE_SCORE_IMPOSSIBLE_RANGE)
...
describe('applyFilter_uncertainty_score', () => {
    ...
    it('keeps_only_high_uncertainty_event_when_min_is_0_63', () => {
        const result = applyFilter(events as any, FILTER_UNCERTAINTY_SCORE_MIN_0_63)
    ...
    it('excludes_high_uncertainty_event_when_max_is_0_38', () => {
        const result = applyFilter(events as any, FILTER_UNCERTAINTY_SCORE_MAX_0_38)
    ...
    it('all_low_uncertainty_events_are_kept_when_max_matches_their_score', () => {
        const result = applyFilter(events as any, FILTER_UNCERTAINTY_SCORE_MAX_0_38)
        const lowUncertaintyCount = events.filter(e => (e.scoring.uncertainty_score ?? Infinity) <= 0.38).length
...
describe('applyFilter_distance_to_coast', () => {
    ...
    it('returns_correct_count_for_distance_range_20_to_26_km', () => {
        const result = applyFilter(events as any, FILTER_DISTANCE_RANGE_20_TO_26_KM)
        const expected = events.filter(e => e.distance_to_coast_km >= 20 && e.distance_to_coast_km <= 26)
...
describe('applyFilter_reason_codes_include', () => {
    ...
    it('keeps_only_event_with_near_coast_reason_code', () => {
        const result = applyFilter(events as any, FILTER_REASON_CODES_INCLUDE_NEAR_COAST)
    ...
    it('keeps_only_event_with_unmatched_to_public_ais_reason_code', () => {
        const result = applyFilter(events as any, FILTER_REASON_CODES_INCLUDE_UNMATCHED)
    ...
    it('include_is_OR_logic_keeping_events_with_any_of_the_listed_codes', () => {
        const result = applyFilter(events as any, FILTER_REASON_CODES_INCLUDE_CARGO_OR_MPA)

        result.forEach(e => {
            const hasMatch = e.scoring.reason_codes?.includes(EReasonCodesStatic.bathymetry_cargo_anomaly_zone)
    ...
    it('returns_empty_array_when_no_event_has_included_reason_code', () => {
        const result = applyFilter(events as any, FILTER_REASON_CODES_INCLUDE_NONEXISTENT)
...
describe('applyFilter_reason_codes_exclude', () => {
    ...
    it('removes_all_cargo_anomaly_events_when_code_is_excluded', () => {
        const result = applyFilter(events as any, FILTER_REASON_CODES_EXCLUDE_CARGO_ANOMALY)

        result.forEach(e => {
            expect(e.scoring.reason_codes).not.toContain('bathymetry_cargo_anomaly_zone')
        })
    })
    ...
    it('removes_near_coast_and_unmatched_events_when_both_codes_excluded', () => {
        const result = applyFilter(events as any, FILTER_REASON_CODES_EXCLUDE_NEAR_COAST_AND_UNMATCHED)
...
describe('applyFilter_bathymetry', () => {
    ...
    it('keeps_only_shallow_event_when_min_is_neg_25', () => {
        ...
        expect(result).toHaveLength(1)
        expect(Number(result[0].context_layers.Bathymetry.enrichments[0].value)).toBeGreaterThanOrEqual(-25)
    })

    it('shallow_event_has_bathymetry_value_of_neg_22', () => {
        const result = applyFilter(events as any, FILTER_BATHYMETRY_MIN_NEG_25)

        expect(Number(result[0].context_layers.Bathymetry.enrichments[0].value)).toBe(-22)
    })
    ...
    it('keeps_only_deep_events_when_max_is_neg_45', () => {
        ...
        result.forEach(e => {
            expect(Number(e.context_layers.Bathymetry.enrichments[0].value)).toBeLessThanOrEqual(-45)
        })
    })

    it('returned_count_for_max_neg_45_matches_manually_filtered_count', () => {
        const result = applyFilter(events as any, FILTER_BATHYMETRY_MAX_NEG_45)
        const expected = events.filter(e => Number(e.context_layers.Bathymetry.enrichments[0]?.value) <= -45)

        expect(result).toHaveLength(expected.length)
    })

    it('all_events_in_range_neg44_to_neg43_satisfy_both_bounds', () => {
        const result = applyFilter(events as any, FILTER_BATHYMETRY_RANGE_NEG44_TO_NEG43)
    ...
    it('event_with_no_bathymetry_enrichments_is_excluded_by_bathymetry_filter', () => {
        ...
        const result = applyFilter(mixedEvents, FILTER_BATHYMETRY_MAX_NEG_45).map(e => e.event_id)
...
describe('applyFilter_combined_filters', () => {
    ...
    it('low_triage_events_without_cargo_anomaly_have_no_cargo_reason_code', () => {
        const result = applyFilter(events as any, FILTER_LOW_TRIAGE_EXCLUDE_CARGO_ANOMALY)
    ...
    it('combined_filters_return_subset_smaller_than_either_filter_alone', () => {
        const byTriageOnly   = applyFilter(events as any, FILTER_TRIAGE_SCORE_MIN_0_73)
    ...
    it('order_of_filter_application_does_not_affect_final_result', () => {
        ...
        expect(ids(resultA as any).sort()).toEqual(byBoth.map(e => e.event_id).sort())
    })
})
