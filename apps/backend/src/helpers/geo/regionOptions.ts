...
import { IGeometry, IPointGeometry, TRegionOption, TRegionOptionFeature } from '@packages/types';
...
const toRegionOption = (
  ...
  try {
    const feature = { type: 'Feature' as const, geometry: a_Geometry, properties: {} };
    return {
      type: 'Feature',
...
const buildEEZOptions = (): TRegionOptionFeature => {
  ...
  const features = all
    .map((f) => toRegionOption(f.geometry, String(f.properties.MRGID), f.properties.GEONAME))
...
export const getRegionOptions = (a_Dataset: EContextLayers): TRegionOptionFeature | undefined => {
