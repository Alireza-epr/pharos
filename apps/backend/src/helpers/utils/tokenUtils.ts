...
export const generateRefreshToken = (a_Payload: JwtPayload) => {
  ...
  const refreshToken = jwt.sign(a_Payload, config.auth.jwt_secret, { expiresIn: refresh_token_expiry });
...
export const logTokenExpiry = (a_Decoded: TDecoded) => {
  if (!(a_Decoded as JwtPayload).exp) {
    log(`Token for user ${(a_Decoded as JwtPayload).username} has no expiry`, ELogType.info);
    return;
  }
  ...
  if (timeLeft <= 0) {
    log(`Token for user ${(a_Decoded as JwtPayload).username} has expired`, ELogType.warn);
  } else {
    log(`Token for user ${(a_Decoded as JwtPayload).username} expires in ${timeLeft} seconds`, ELogType.info);
  }
}
...
export const getUser = (a_Username: string, a_Role: ERequestUserRole): IUser => {
  ...
  const refreshToken = generateRefreshToken({ username: a_Username, role: a_Role });
  const user = {
    username: a_Username,
    role: a_Role,
    accessToken,
