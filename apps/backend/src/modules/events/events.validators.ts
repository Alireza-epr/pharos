import { IFilteringParams, IThresholdConfig, IValidationErrorDetail, IValidationResult } from '@packages/types';
import { EFormat, EGeoJSONGeometryType, EGroupBy, EHotspotTimeBins, ERegionBufferOperations, ERegionBufferUnits, ERegionDatasets, EResponseError, ESpatialResolution, EStatusCode, ETemporalResolution } from '@packages/enum';
import { isBoolean, isNumber, isObject, isString } from '@packages/utils';
import { addError, validateRequiredObject } from '../../helpers/utils/controllerUtils';
...
const validateGeoJSON = (
  ...
  if (
    a_Geojson.coordinates === undefined ||
    a_Geojson.coordinates === null
  ) {
    addError(
      a_Errors,
...
const validateHotspot = (
  ...
  if (!isNumber(a_Hotspot.resolution)) {
    ...
    a_Hotspot.resolution < 0 ||
    a_Hotspot.resolution > 15
  ) {
    addError(
      a_Errors,
...
const validateRegionFields = (
  ...
  if (
    a_Body["region.id"] !== undefined &&
    !isString(a_Body["region.id"])
  ) {
    addError(
      a_Errors,
      EResponseError.INVALID_STRING,
  ...
  if (
    ...
    !REGION_BUFFER_OPERATIONS.includes(
      a_Body["region.bufferOperation"]
    )
  ) {
    addError(
      a_Errors,
...
const validateBoolean = (
  ...
  if (
    !(a_Value === "true" || a_Value === "false" || isBoolean(a_Value))
  ) {
    addError(
      a_Errors,
      EResponseError.INVALID_BOOLEAN,
...
const validateNumber = (
  ...
  const parsed =
    typeof a_Value === "string" ? Number(a_Value) : a_Value;
