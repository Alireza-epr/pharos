...
import {
  ...
  Observer,
  Subject,
} from '../helpers/utils/servicesUtils';
...
export class IOService extends Subject<any> {
  ...
  async sendMessage(a_Message: IMessage): Promise<IMessage> {
    ...
    return new Promise<IMessage>((resolve, reject) => {
      const thisMessageObserver = new Observer<IMessage>(
        (a_ReceivedMessage) => {
          if (
            a_ReceivedMessage.header.refId ==
              thisMessageWithRefId.header.refId &&
            a_ReceivedMessage.header.sessionId ==
              thisMessageWithRefId.header.sessionId
          ) {
            this.receiveMessageSubject.unsubscribe(thisMessageObserver);
            this.requestsQueue = this.requestsQueue.filter(
              (m) => m.header.refId !== a_Message.header.refId,
            );
            if (a_ReceivedMessage.header.status == EMessageStatus.failed) {
              if (a_ReceivedMessage.body.error) {
                reject(
                  `[IOService] Error refId ${a_ReceivedMessage.header.refId}: ${a_ReceivedMessage.body.error}`,
                );
              } else {
                reject(
                  `[IOService] Failed to get response refId ${a_ReceivedMessage.header.refId}, sessionId ${a_ReceivedMessage.header.sessionId}`,
                );
              }
            }
      ...
      setTimeout(() => {
        ...
        reject(
          `[IOService] Failed to get response refId ${a_Message.header.refId}, sessionId ${a_Message.header.sessionId} after ${this.responseTimeout / 1000}s`,
        );
