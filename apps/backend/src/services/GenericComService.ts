...
import { IMessage, IReceiverEmitterArgs, TMessageBody } from "../helpers/types/ioTypes";
...
export class GenericComService {
    ...
    constructor(a_RouteService: RouteService, a_IOService: IOService) {
        ...
        const classIds = Object.values(EClassId).filter(v => typeof v === "number")
        for (const classId of classIds) {
            //const key = `${a_SessionId}_${classId}`
            const key = `${0}_${classId}`
            this.ioService.receiverEmitter.on(key, (...args: IReceiverEmitterArgs) => {
                log(`[GenericComService] Message for ClassId: ${classId}`, ELogType.info)
                log(`[GenericComService] Message Header: ${JSON.stringify(args[0])}`, ELogType.info)
                log(`[GenericComService] Message Body: ${JSON.stringify(args[1])}`, ELogType.info)
            })
        }
    ...
    async execute<T>(
        ...
        throw new Error("[GenericComService] Failed to send the message: "+ JSON.stringify(message))
    }
