...
export default class RouteService extends Subject<IMessage> {
    ...
    public async handleSendingMessage(a_Message: IMessage) {
        ...
        log(`[RouteService] Send the message: ${JSON.stringify(a_Message)}`, ELogType.info)
        let repository
        switch(a_Message.header.repository){
            case ERepositoryKey.gfw:
                ...
                break;
            default:
                this.pushErrorToBody(a_Message, `[RouteService] Repository not defined: ${a_Message.header.repository}`)
                break;
        }
    }
    ...
    public handleReceivingMessage(a_Message: IMessage) {
        ...
        log(`[RouteService] Message is received: ${JSON.stringify(receivedMessage)}`, ELogType.info)
