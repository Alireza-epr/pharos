...
import { getEnvVariable, isDevelopment } from '../helpers/utils/controllerUtils';
...
export const corsCheck = (a_Req: Request, a_Res: Response, a_Next: NextFunction) => {
    ...
    const methodsString = a_Req.headers['access-control-request-method'] || a_Req.method
    ...
    // Chech Headers
    const allowedHeaders = process.env.CORS_HEADERS?.split(",").map( h => h.trim().toLowerCase())
...
export const cors = (a_Req: Request, a_Res: Response, a_Next: NextFunction) => {
    ...
    a_Res.header("Access-Control-Allow-Credentials", process.env.CORS_CREDENTIALS );
    ...
    a_Res.header("Access-Control-Expose-Headers", process.env.CORS_EXPOSE_HEADERS);
