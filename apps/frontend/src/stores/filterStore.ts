...
import { IFilteringParams, IFilteringParamsUI, T4wingsSource, TDatasetVersion, TFilterKey, TSourceKey } from '@packages/types';
import { IFilterStoreActions, IFilterStoreStates } from '../helpers/types/storeTypes';
import { E4wingsDatasets, E4wingsDatasetsUI, EMatchFilter, ENeuralVesselType, TMatchFilter } from '@packages/enum';
import { getFlags, getGearTypes, getMatched, getMinimumDistanceFromPorts, getNeuralVesselType, getSpeeds, getVesselId, getVesselTypes } from '@/helpers/utils/queryUtils';
...
const FILTER_BUILDERS: Record<E4wingsDatasets, ((ui: IFilteringParamsUI) => string)[]> = {
    [E4wingsDatasets.SARVesselDetections]: [
...
export const useFilterStore = create<IFilterStoreStates & IFilterStoreActions>(
    combine(
        ...
        (set, get) => ({
            setFilters: (a_Value) => set((state) => ({ filters: typeof a_Value === 'function' ? a_Value(state.filters) : a_Value })),
            setFiltersUI: (a_Value) => set((state) => ({ filtersUI: typeof a_Value === 'function' ? a_Value(state.filtersUI) : a_Value })),
            getSources: () => {
                const { datasets } = get().filtersUI
                let datasetNum = 0
                const datasetsObject = Object.entries(datasets).reduce((acc: Record<TSourceKey, T4wingsSource>, [ds, value]) => {
                    if (value.active) {
            ...
            getFilter: () => {
                const { filtersUI } = get()
                const { datasets } = get().filtersUI
                const active = (Object.entries(datasets) as [E4wingsDatasets, { active: boolean; version: TDatasetVersion }][]).filter(([_, value]) => value.active)
