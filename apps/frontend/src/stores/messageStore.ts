...
export const useMessageStore = create<
  IMessageStoreStates & IMessageStoreActions
>()(
  persist(
    combine(
      {
      ...
      (set) => ({
        setMessage: (a_Value) =>
          set((state) => ({
            info: typeof a_Value === 'function' ? a_Value(state.info) : a_Value,
          })),
        setWarn: (a_Value) =>
          set((state) => ({
            warn: typeof a_Value === 'function' ? a_Value(state.warn) : a_Value,
          })),
        setError: (a_Value) =>
          set((state) => ({
            error:
              typeof a_Value === 'function' ? a_Value(state.error) : a_Value,
          })),
      }),
    ),
    {
